...
class ActorNetwork_R(nn.Module):
    ...
    def __init__(self, observation_size, action_size, use_batch_norm, seed,
        ...
        self.lstm2 = nn.LSTM(fc1_units, fc2_units, bias=use_bias, batch_first=True)
    ...
    def forward(self, observation, hidden):
        """ map a states to action values
        :param observation: shape == (batch, length, observation_size)
        :param hidden h: shape == (1, fc2_units)
        :param hidden c: shape == (1, fc2_units)
        :return: action values
        """
        ...
        if self.use_batch_norm:
            ...
            x, hidden = self.lstm2(self.bn2(x), hidden)
            ...
            x = F.relu(self.fc3(x))
            return torch.tanh(self.fc4(x)), hidden

    def reset_parameters(self):
        self.fc1.weight.data.uniform_(*hidden_init(self.fc1))
        self.lstm2.weight.data.uniform_(*hidden_init(self.lstm2)) #先暂时这么写
        self.fc3.weight.data.uniform_(*hidden_init(self.fc3))
        self.fc4.weight.data.uniform_(-3e-3, 3e-3)


class CriticNetwork_R(nn.Module):
    ...
    def forward(self, observation, action, hidden):
